...
class ShellTest(testtools.TestCase):
    ...
    def shell(self, argstr, check=False):
        ...
        try:
            ...
            _shell = openstack_shell.MagnetoDBShell('1', 'keyvalue',
    ...
    def test_auth(self):
        magnetodb_shell = openstack_shell.MagnetoDBShell('1',
            openstack_shell.API_NAME, openstack_shell.COMMANDS)
    ...
    def test_main_with_unicode(self):
        ...
        manager_p.start()
        self.addCleanup(manager_p.stop)
        qshell_mock = openstack_shell.MagnetoDBShell('1',
            openstack_shell.API_NAME, openstack_shell.COMMANDS)
        qshell_mock.run.return_value = 0
        unicode_text = u'\u7f51\u7edc'
        argv = ['table-list', unicode_text, unicode_text.encode('utf-8')]
        ret = openstack_shell.main(argv=argv)
        qshell_mock.run.assert_called_once_with([u'table-list', unicode_text, unicode_text])
        self.assertEqual(ret, 0)

    def test_endpoint_option(self):
        shell = openstack_shell.MagnetoDBShell('1',
            openstack_shell.API_NAME, openstack_shell.COMMANDS)
